...
class Tour {
	 ...
	 */
	function __construct($id = null, $arriveeHeure, $codeprogramme, $dateDebut, $dateFin, $dateLimiteInscr, $departHeure, $descente, $description_de, $description_fr, $difficulte, $duree, $idxArriveeLocalite, $idxAssistant, $idxDepartLocalite, $idxGuide, $idxTypeTour, $idxTypeTransport, $infromation_de, $information_fr, $inscriptionMax, $lienCarte, $lieuRDV, $montee, $prixMax, $prixMin, $soustitre, $status, $titre, $transportArrivee, $transportDepart) {
		$this->setId ( $id );
	...
	/**
	 * Method that Gets all Tours by idTour
	 * 
	 * @param unknown $id        	
	 * @return boolean|Tour
	 */
	public static function connectbyId($id) {
		...
		return new Tour ( $row ['idTour'], $row ['arriveeHeure'], $row ['codeprogramme'], $row ['dateDebut'], $row ['dateFin'], $row ['dateLimiteInscr'], $row ['departHeure'], $row ['descente'], $row ['description_de'], $row ['description_fr'], $row ['difficulte'], $row ['duree'], $row ['idxArriveeLocalite'], $row ['idxAssistant'], $row ['idxDepartLocalite'], $row ['idxGuide'], $row ['idxTypeTour'], $row ['idxTypeTransport'], $row ['infromation_de'], $row ['information_fr'], $row ['inscriptionMax'], $row ['lienCarte'], $row ['lieu'], $row ['montee'], $row ['prixMax'], $row ['prixMin'], $row ['soustitre'], $row ['status'], $row ['titre'], $row ['transportArrivee'], $row ['transportDepart'] );
	}
	
	/**
	 * Method that connects to all tours by two diff id's (explained in the comment)
	 * 
	 * @param unknown $idTour1        	
	 * @param unknown $idTour2        	
	 * @return boolean|Tour
	 */
	public static function connectTour($idTour1, $idTour2) // if programm 1 or and 2 // proposal 6 by TYPE TOUR
{
		...
		if ($nbr == 0) {
			...
			while ( $row = $result->fetch () ) {
				$resultArray [] = new Tour ( $row ['idTour'], $row ['arriveeHeure'], $row ['codeprogramme'], $row ['dateDebut'], $row ['dateFin'], $row ['dateLimiteInscr'], $row ['departHeure'], $row ['descente'], $row ['description_de'], $row ['description_fr'], $row ['difficulte'], $row ['duree'], $row ['idxArriveeLocalite'], $row ['idxAssistant'], $row ['idxDepartLocalite'], $row ['idxGuide'], $row ['idxTypeTour'], $row ['idxTypeTransport'], $row ['information_de'], $row ['information_fr'], $row ['inscriptionMax'], $row ['lienCarte'], $row ['lieuRDV'], $row ['montee'], $row ['prixMax'], $row ['prixMin'], $row ['soustitre'], $row ['status'], $row ['titre'], $row ['transportArrivee'], $row ['transportDepart'] );
			}
	...
	/**
	 * Method that returns a query for selecting all Tours
	 * 
	 * @return string
	 */
	...
	/**
	 * Method that returns a query for one Tour through id
	 * 
	 * @param unknown $idTour1        	
	 * @return string
	 */
	...
	/**
	 * Method that returns a query for selecting information with two id's
	 * 
	 * @param unknown $idTour1        	
	 * @param unknown $idTour2        	
	 * @return string
	...
	/**
	 * Method used on search function for getting the results from database with the three given properties
	 * 
	...
	/**
	 * Method used for search funstionality for getting restults with the given text propoerty
	 * 
	 * @param unknown $tx        	
	 * @return boolean|Tour
	 */
	...
	/**
	 * Method used to connect for the program of the user using inscription and type
	 * 
	 * @param unknown $inscriptionArray        	
	 * @param unknown $type        	
	 * @return Tour
	 */
	public static function connectForMyProgramm($inscriptionArray, $type) // 0=>Register 1=>Favoris 2=>evalutation
{
	...
	/**
	 * Method that creates condition for register
	 * 
	 * @param unknown $inscriptionArray        	
	 * @return string
	 */
	...
	/**
	 * Method that creates contition for favoorites
	 * 
	 * @param unknown $inscriptionArray        	
	 * @return string
	 */
	...
	/**
	 * Method used to connect to registered tour
	 * 
	 * @param unknown $inscriptionArray        	
	 * @return Tour
	 */
	...
	/**
	 * Method for creating a condition
	 * 
	 * @param unknown $inscriptionArray        	
	 * @return string
	 */
	...
	/**
	 * Method used for creating new tour
	 * 
	 * @param unknown $tour        	
	 * @return boolean
	 */
	...
	/**
	 * Method for deleting tour through id
	 * 
	 * @param unknown $id        	
	 * @return boolean
	 */
	...
	/**
	 * Method that retrieves all tours
	 * 
	 * @return Tour
	 */
	public static function connectToAll() {
		...
		$result = MySqlConn::getInstance ()->selectDB ( $query );
		while ( $row = $result->fetch () ) {
			$resultArray [$row ['idTour']] = new Tour ( $row ['idTour'], $row ['arriveeHeure'], $row ['codeprogramme'], $row ['dateDebut'], $row ['dateFin'], $row ['dateLimiteInscr'], $row ['departHeure'], $row ['descente'], $row ['description_de'], $row ['description_fr'], $row ['difficulte'], $row ['duree'], $row ['idxArriveeLocalite'], $row ['idxAssistant'], $row ['idxDepartLocalite'], $row ['idxGuide'], $row ['idxTypeTour'], $row ['idxTypeTransport'], $row ['information_de'], $row ['information_fr'], $row ['inscriptionMax'], $row ['lienCarte'], $row ['lieuRDV'], $row ['montee'], $row ['prixMax'], $row ['prixMin'], $row ['soustitre'], $row ['status'], $row ['titre'], $row ['transportArrivee'], $row ['transportDepart'] );
		}
	...
	/**
	 * Method for getting the registered members for the selected tour through id of tour
	 * 
	 * @param unknown $idTour        	
	 * @return boolean|Personne
	 */
	public static function get_registered_members($idTour) {
		...
		if ($row_no == 0) {
			...
			while ( $row = $result->fetch () ) {
				$resultArray [] = new Personne ( $row ['idPersonne'], $row ['prenom'], $row ['nom'], $row ['email'], $row ['motDePasse'], $row ['idxLangue'], $row ['estActif'], $row ['numMembre'], $row ['role'], $row ['portable'], $row ['telephone'], $row ['adresse'], $row ['Localite'], $row ['NPA'], $row ['idxAbonnement'] );
			}
			$result->closeCursor ();
			return $resultArray;
		}
